...
Rid RmFileHandle::insert_record(char* buf, Context* context) {
    ...
    if (slot_no == file_hdr_.num_records_per_page) {
        buffer_pool_manager_->unpin_page(PageId{fd_, page_handle.page->get_page_id().page_no}, false);
        throw InternalError("RmFileHandle::insert_record Error");
    }
...
void RmFileHandle::insert_record(const Rid& rid, char* buf) {
    RmPageHandle page_handle = fetch_page_handle(rid.page_no);
    bool slot_was_empty = !Bitmap::is_set(page_handle.bitmap, rid.slot_no);
        ...
        if (file_hdr_.first_free_page_no == rid.page_no) {
            ...
            while (prev_page_no != RM_NO_PAGE) {
                ...
                if (prev_page_handle.page_hdr->next_free_page_no == rid.page_no) {
                    ...
                    buffer_pool_manager_->unpin_page(PageId{fd_, prev_page_no}, true);
                    page_handle.page_hdr->next_free_page_no = RM_NO_PAGE;
                    break;
                }

                int next_page_no = prev_page_handle.page_hdr->next_free_page_no;
                buffer_pool_manager_->unpin_page(PageId{fd_, prev_page_no}, false);
